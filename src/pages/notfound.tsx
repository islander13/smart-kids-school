...
export default function NotFound() {
  ...
  useEffect(() => {
    document.documentElement.lang = { FR: 'fr', EN: 'en', DE: 'de' }[locale];
    document.title = `${t.title} | Smart Kids School`;
    ...
    return () => {
      // Une page 404 ne doit jamais laisser le site en noindex si l'utilisateur
      // navigue ensuite vers une page indexable sans que celle-ci ne réécrive
      // la balise à temps (chaque page le fait déjà, mais on reste prudent).
    ...
    <div className={`min-h-screen flex flex-col font-['Inter',sans-serif] ${darkMode ? 'bg-gray-950 text-white' : 'bg-white text-gray-900'}`}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 text-center relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-lg">
          ...
          <h1 className={`text-3xl sm:text-4xl font-bold mb-4 ${darkMode ? 'text-white' : 'text-gray-900'}`}>{t.title}</h1>
          <p className={`text-base sm:text-lg mb-2 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>{t.desc}</p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <a href={lp('/')} className="bg-[#232999] hover:bg-[#1a1f7a] text-white px-8 py-3 rounded-full font-semibold transition-all hover:shadow-lg">{t.home}</a>
            <a href={lp('/tarifs')} className={`px-8 py-3 rounded-full font-semibold border-2 transition-all ${darkMode ? 'border-gray-600 text-gray-300 hover:border-indigo-400' : 'border-gray-200 text-gray-700 hover:border-[#232999]'}`}>{t.pricing}</a>
          </div>
        </div>
      </div>
      <div className={`text-center py-4 text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>© 2022–2026 Smart Kids School</div>
    </div>
  );
}
