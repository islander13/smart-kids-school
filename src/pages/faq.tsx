...
const T: Record<Lang, {
  ...
  FR: {
    ...
    pricing: [
      ...
      { q: "Peut-on essayer avant de s'engager ?", a: "Oui : nous proposons un cours d'essai gratuit de 30 minutes, sans engagement. C'est l'occasion d'évaluer le niveau de votre enfant avec un enseignant et de lui proposer le programme le plus adapté, avant toute inscription." },
  ...
  EN: {
    ...
    pricing: [
      ...
      { q: "Can I try it before committing?", a: "Yes: we offer a free 30-minute trial lesson, with no commitment. It's a chance to have a teacher evaluate your child's level and recommend the best-fitting programme, before any enrollment." },
  ...
  DE: {
    ...
    pricing: [
      ...
      { q: "Kann ich es vor der Anmeldung ausprobieren?", a: "Ja: Wir bieten eine kostenlose, unverbindliche Probestunde von 30 Minuten an. Dabei schätzt eine Lehrperson das Niveau Ihres Kindes ein und empfiehlt das passende Programm — ganz ohne Anmeldung." },
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="hidden md:flex items-center gap-8 ml-8">
              <a href={lp('/')} className={`text-sm font-medium transition-colors whitespace-nowrap ${darkMode ? 'text-gray-300 hover:text-indigo-400' : 'text-gray-700 hover:text-indigo-700'}`}>{t.navHome}</a>
              <a href={lp('/#parcours')} className={`text-sm font-medium transition-colors whitespace-nowrap ${darkMode ? 'text-gray-300 hover:text-indigo-400' : 'text-gray-700 hover:text-indigo-700'}`}>{t.navProgramme}</a>
        ...
        {mobileMenuOpen && (
          <div className={`md:hidden border-t ${darkMode ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-100'}`}>
            <div className="px-4 py-4 space-y-3">
              <a href={lp('/')} onClick={() => setMobileMenuOpen(false)} className={`block text-sm font-medium py-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{t.navHome}</a>
              <a href={lp('/#parcours')} onClick={() => setMobileMenuOpen(false)} className={`block text-sm font-medium py-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{t.navProgramme}</a>
              <a href={lp('/tarifs')} onClick={() => setMobileMenuOpen(false)} className={`block text-sm font-medium py-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{t.navTarifs}</a>
              <a href={lp('/stages')} onClick={() => setMobileMenuOpen(false)} className={`block text-sm font-medium py-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{t.navStages}</a>
              <a href={lp('/blog')} onClick={() => setMobileMenuOpen(false)} className={`block text-sm font-medium py-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{t.navBlog}</a>
              {ESPACE_NAV_VISIBLE && (
                <a href={lp('/espace')} onClick={() => setMobileMenuOpen(false)} className={`block text-sm font-medium py-2 ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>{t.navEspace}</a>
              )}
              <div className="flex gap-2 py-2">
                {(['FR', 'EN', 'DE'] as Lang[]).map(lang => (
                  <button key={lang} onClick={() => { setCurrentLang(lang); navigate(localizedPath('/faq', lang)); setMobileMenuOpen(false); }} className={`px-4 py-2 rounded-lg text-sm font-medium cursor-pointer ${currentLang === lang ? 'bg-[#232999] text-white' : darkMode ? 'bg-gray-800 text-gray-300' : 'bg-gray-100 text-gray-700'}`}>{lang}</button>
                ))}
              </div>
              <a href={lp('/tarifs')} onClick={() => setMobileMenuOpen(false)} className="block w-full bg-[#232999] text-white px-6 py-3 rounded-full text-sm font-semibold text-center">{t.navEnroll}</a>
            </div>
          </div>
        )}
      </nav>
      ...
      <section className="py-16 px-4 bg-gradient-to-br from-[#232999] via-[#1e2470] to-[#171b54] text-white">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <a href="https://wa.me/41774768492" target="_blank" rel="noopener noreferrer" className="bg-[#1f7a44] hover:bg-[#18613a] text-white px-8 py-3 rounded-full font-bold transition-all">{t.ctaWhatsapp}</a>
          </div>
        </div>
      </section>
